...
function FC:new(opts)
    ...
    -- onNesFrame:每个 NES 帧(60Hz)的逻辑回调,不随渲染帧率波动。
    -- 用于连发/turbo 等必须按 NES 时间线推进的逻辑(渲染分离后不能再借渲染回调计时)。
    fc.onNesFrame = opts.onNesFrame or function() end
    ...
    fc.isRunning = false
    fc.frameCount = 0
    -- 声音/渲染分离:deferPresent=true 时 frame() 不在帧内渲染,改由 presentDeferred
    -- 每 WoW 帧渲染一次最新帧(仅快照路径生效)。默认 false 保持旧的逐帧渲染行为,
    -- 测试/黄金快照走默认路径,逐帧 framebuffer 不变。
    fc.deferPresent = false
    fc._pendingPresent = false
    fc.fps = 0
    fc.lastFpsTime = 0
    fc.frameTime = 0
...
function FC:loadROM(data)
    local success, err = pcall(function()
        ...
        -- CPU 取指/ROM 读快速路径:换 ROM 时先清旧视图,只有支持的 mapper(NROM)
        -- 重新铺设;bank 切换型 mapper 不提供 → 自动退回 memoryMapperLoad 慢路径。
        self.cpu.prgDirect = nil
        if self.mmap.buildCpuFastPath then
            self.mmap:buildCpuFastPath(self.cpu)
        end
    end)
        ...
        while cycleCount < CYCLES_PER_FRAME do
            if cpu.cyclesToHalt == 0 then
                ...
                if ppu.frameEnded then
                    ...
                    -- 渲染调度:
                    -- deferPresent 且快照路径(非逐扫描线)时,本帧只标记"有新画面待渲染",
                    -- renderFrame + present 交给外部 present 驱动(每 WoW 帧一次),避免
                    -- catchup 在一个 WoW 帧内重复重绘、拖慢并抖动 apu:tick 的墙钟节奏
                    -- (声音帧与渲染帧分离)。逐扫描线模式渲染与模拟交织,必须帧内收尾,走旧路径。
                    if self.deferPresent and not ppu._perScanline then
                        self._pendingPresent = true
                    else
                    ...
                    -- 每个 NES 帧的逻辑回调(连发/turbo 等按 NES 时间线工作,不随渲染帧率
                    -- 波动)。放在 tick 之后、break 之前。
                    self.onNesFrame()
...
-- 延迟渲染:deferPresent 模式下 frame() 不在帧内渲染,只标记 _pendingPresent。
-- 由主循环每个 WoW 帧调用一次,渲染并 present 当前(最近完成帧)的 PPU 状态。
-- 渲染帧率因此 = WoW 帧率,与 60Hz 的 NES 模拟/apu:tick 解耦:移动时即使渲染重,
-- 也只画一次最新帧,不在 catchup 里重复重绘,声音节奏不被拖慢。
-- 仅快照路径使用,延迟渲染读到的是上一帧 NMI 后的最终状态(startFrame 不改渲染状态),正确。
function FC:presentDeferred()
    if not self._pendingPresent then return false end
    self._pendingPresent = false

    local ppu = self.ppu
    local prof = self._prof
    local t_render_start = _profstop()
    ppu:renderFrame()
    if prof then prof.ms_render = prof.ms_render + (_profstop() - t_render_start) end
    local t_present_start = _profstop()
    self.onFrame(ppu.buffer, ppu)
    if prof then prof.ms_present = prof.ms_present + (_profstop() - t_present_start) end
    return true
end
