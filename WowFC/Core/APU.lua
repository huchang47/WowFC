...
--   noteOn        : "重新发声"信号(note-on)。写 length 高位寄存器($4003/$4007/$400B,
--                   通道使能)即代表一次重新敲击,即使音高与上一帧相同也应重触发一次。
--                   修复(apu-same-tone-retrigger):tick 消费该标志后清零,使"重新敲击的
--                   同音"能正常发声,而"持续按住的同音"(不写 length 高位)仍不会每帧重 play。
...
function APU:writeRegister(address, value)
    ...
    if address == 0x4002 or address == 0x4003 then
        ...
        if address == 0x4003 and ch.pulse1.enabled then
            ...
            -- 修复(apu-same-tone-retrigger):写 length 高位 = 一次 note-on(重新敲击),
            -- 标记重新发声,使 tick 即使音高未变也重触发一次。
            ch.pulse1.noteOn = true
        end
...
function APU:tick()
    ...
    for _, name in ipairs(CHANNEL_ORDER) do
        ...
        -- 触发条件:静音→发声(prev==nil 且 now~=nil)、持续发声跨半音(两者非 nil 且不等),
        -- 或本帧收到 note-on 重新敲击信号(channel.noteOn,修复 apu-same-tone-retrigger ——
        -- 即使音高与上一帧相同也重触发一次,使"重新敲击的同音连续发声"不再被过滤)。
        local needTrigger = (nowTone ~= nil) and ((nowTone ~= prevTone) or channel.noteOn)
        ...
        if needTrigger and canPlay then
            ...
            if tickBudgetOk then
                ...
                if path ~= nil then
                    -- 修复(apu-sound-leak):覆盖前先停止该通道上一个仍持有的句柄,
                    -- 保证任一通道任意时刻最多只有一个正在播放的实例,避免句柄/内存无界泄漏。
                    if channel.handle ~= nil then
        ...
        -- 消费并清除 note-on 信号:该信号仅在写入 length 高位的那一帧有效,无论本帧
        -- 是否实际触发(可能被 maxTriggersPerTick 节流抑制),都清零以免后续帧反复重触发。
...
function APU:setEnabled(on)
    ...
    if wasEnabled and not newEnabled then
        for _, name in ipairs(CHANNEL_ORDER) do
            ...
            -- 清除残留的 note-on 信号:关闭期间 tick 短路不消费,避免重开后误触发一次。
            channel.noteOn = false
        end
    end
end
